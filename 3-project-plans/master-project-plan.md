...
| 030 | [[030-task-crud-ordering\|Task CRUD & Ordering]] | Pending | Create, delete, reorder tasks (+ empty task state behavior) | Task creation, task deletion, task order sorting (drag-and-drop or buttons). Empty task state behavior (what happens when a list has no tasks). No formatting focus. |
